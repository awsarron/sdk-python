...
from strands.multi_agent.graph import (
    Graph, GraphBuilder, GraphNode, GraphEdge, GraphState, GraphResult, Status
)
...
def mock_agent():
    ...
    mock_result = AgentResult(
        ...
        state={},
        metrics=Mock(accumulated_usage={"inputTokens": 10, "outputTokens": 20, "totalTokens": 30},
...
async def test_comprehensive_graph_execution_with_diverse_topology(mock_agent, mock_multi_agent):
    ...
    agent2_result = AgentResult(
        ...
        stop_reason="end_turn", state={}, metrics=Mock(
    ...
    with patch('strands.multi_agent.graph.time.time') as mock_time:
    ...
    expected_edges = {("start_agent", "multi_node"), ("start_agent", "agent_2"), 
                     ("multi_node", "final_node"), ("agent_2", "final_node")}
