...
class Swarm(MultiAgentBase):
    ...
    def _setup_swarm(self) -> None:
        ...
        logger.debug("agents=<%s> | initialized swarm with agents", agent_names)
    ...
    def _handle_completion(self, result: str, summary: str = "") -> None:
        ...
        logger.debug("result=<%s> | swarm task completed", result)
    ...
    def _handle_user_handoff(
        ...
        logger.debug("agent=<%s>, question=<%s> | user handoff from agent", self.swarm_state.current_agent, question)
    ...
    async def _continue_execution(self) -> List[MultiAgentResult]:
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    logger.debug("status=<%s> | agent execution completed", result.status)
        ...
        logger.debug("status=<%s> | swarm execution completed", self.swarm_state.completion_status.value)
        logger.debug(
    ...
    async def execute(self, task: str) -> List[MultiAgentResult]:
        ...
        logger.debug("current_agent=<%s> | starting safe swarm execution with agent", self.swarm_state.current_agent)
        logger.debug(
    ...
    async def resume_from_user_input(self, user_response: str) -> List[MultiAgentResult]:
        ...
        logger.debug("user_response=<%s> | resuming swarm with user response", user_response)
