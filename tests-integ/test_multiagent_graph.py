...
async def test_graph_execution(math_agent, summary_agent, validation_agent, nested_computation_graph):
    ...
    def should_validate(state):
        ...
        return "computation_subgraph" in state.completed_nodes

    def should_skip_extra_summary(state):
        """Condition to potentially skip additional summary."""
    ...
    builder.add_edge("primary_summary", "secondary_summary", condition=should_skip_extra_summary)  # Conditional (false)
    ...
    assert result.execution_order == ["computation_subgraph", "secondary_math", "validator", "primary_summary"]
    ...
    nested_result = result.results["computation_subgraph"]
